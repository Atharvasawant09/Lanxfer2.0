...
#### 4. SSL Certificates (Zero-Config)
SSL Certificates (`cert.pem` and `key.pem`) are beautifully auto-generated by the Python backend on the first launch using the `cryptography` library. No manual OpenSSL setup is required!
